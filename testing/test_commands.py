...
class CommandsTest(unittest.TestCase):
    ...
    @test_command(command=commands.create_time,
                  data=test_data.create_time_data)
    def test_create_time(self, data, result):
        assert result == data.expected_response

    @test_command(command=commands.update_time,
                  data=test_data.update_time_data)
    def test_update_time(self, data, result):
        assert result == data.expected_response

    @test_command(command=commands.get_times,
                  data=test_data.get_times_no_uuid_data)
    def test_get_times_no_uuid(self, data, result):
        assert result == data.expected_response

    @test_command(command=commands.get_times,
                  data=test_data.get_times_uuid_data)
    def test_get_times_uuid(self, data, result):
        assert result == data.expected_response

    @test_command(command=commands.sum_times,
                  data=test_data.sum_times_data)
    def test_sum_times(self, data, result):
        assert result == data.expected_response

    @test_command(command=commands.delete_time,
                  data=test_data.delete_time_no_data)
    def test_delete_time_no(self, data, result):
        assert result == data.expected_response

    @test_command(command=commands.delete_time,
                  data=test_data.delete_time_data)
    def test_delete_time(self, data, result):
        assert result == data.expected_response

    @test_command(command=commands.create_project,
                  data=test_data.create_project_data)
    def test_create_project(self, data, result):
        assert result == data.expected_response

    @test_command(command=commands.update_project,
                  data=test_data.update_project_data)
    def test_update_project(self, data, result):
        assert result == data.expected_response

    @test_command(command=commands.get_projects,
                  data=test_data.get_projects_no_slug_data)
    def test_get_projects_no_slug(self, data, result):
        assert result == data.expected_response

    @test_command(command=commands.get_projects,
                  data=test_data.get_projects_slug_data)
    def test_get_projects_slug(self, data, result):
        assert result == data.expected_response

    @test_command(command=commands.delete_project,
                  data=test_data.delete_project_no_data)
    def test_delete_project_no(self, data, result):
        assert result == data.expected_response

    @test_command(command=commands.delete_project,
                  data=test_data.delete_project_data)
    def test_delete_project(self, data, result):
        assert result == data.expected_response

    @test_command(command=commands.create_activity,
                  data=test_data.create_activity_data)
    def test_create_activity(self, data, result):
        assert result == data.expected_response

    @test_command(command=commands.update_activity,
                  data=test_data.update_activity_data)
    def test_update_activity(self, data, result):
        assert result == data.expected_response

    @test_command(command=commands.get_activities,
                  data=test_data.get_activities_no_slug_data)
    def test_get_activities_no_slug(self, data, result):
        assert result == data.expected_response

    @test_command(command=commands.get_activities,
                  data=test_data.get_activities_slug_data)
    def test_get_activities_slug(self, data, result):
        assert result == data.expected_response

    @test_command(command=commands.delete_activity,
                  data=test_data.delete_activity_no_data)
    def test_delete_activity_no(self, data, result):
        assert result == data.expected_response

    @test_command(command=commands.delete_activity,
                  data=test_data.delete_activity_data)
    def test_delete_activity(self, data, result):
        assert result == data.expected_response

    @test_command(command=commands.create_user,
                  data=test_data.create_user_data)
    def test_create_user(self, data, result):
        assert result == data.expected_response

    @test_command(command=commands.update_user,
                  data=test_data.update_user_data)
    def test_update_user(self, data, result):
        assert result == data.expected_response

    @test_command(command=commands.get_users,
                  data=test_data.get_users_no_slug_data)
    def test_get_users_no_slug(self, data, result):
        assert result == data.expected_response

    @test_command(command=commands.get_users,
                  data=test_data.get_users_slug_data)
    def test_get_users_slug(self, data, result):
        assert result == data.expected_response

    @test_command(command=commands.delete_user,
                  data=test_data.delete_user_no_data)
    def test_delete_user_no(self, data, result):
        assert result == data.expected_response

    @test_command(command=commands.delete_user,
                  data=test_data.delete_user_data)
